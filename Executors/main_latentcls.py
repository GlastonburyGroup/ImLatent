...
def getARGSParser():
    ...
    #Basic params
    parser.add_argument('--taskID', action="store", type=int, default=2, help="0: Reconstruction-based latent extraction using AEs and VAEs, 1: Latent extraction using diffusion autoencoder, 2: Latent classification models (utilised for latent manipulation for DiffAE)")
    parser.add_argument('--trainID', action="store", default="provafp16_diffAE", help="The name of the training session. run_prefix (from datainfo json) and foldID will be added as prefix and suffix")
    ...
    parser.add_argument('--batch_size', action="store", default=100, type=int, help="Actual batch size for the model. This can be modified as required, just control the effective_batch_size for comparibility")
    parser.add_argument('--effective_batch_size', action="store", default=100, type=int, help="To make runs with different BS comparible, keep this constant. accumulate_gradbatch would be this divided by BS. (Original paper: same as batch size)")
    ...
    parser.add_argument('--n_train_batches', action="store", default=1.0, type=float, help="How much of training dataset to use, can be float between 0.0 and 1.0, or an integer (float = fraction, int = num_batches)")
    parser.add_argument('--n_val_batches', action="store", default=1.0, type=float, help="How much of validation dataset to use, can be float between 0.0 and 1.0, or an integer (float = fraction, int = num_batches)")
    ...
    parser.add_argument('--datajson_path', action="store", default="Configs/UKBB208/dataNpath_F20208_time2slc_4ChTransverse_Crop128_conf_3pheno.json", help="Relative path, from the project root, to the dataNpath json file, containing the paths and other data related info")
    ...
    parser.add_argument('--ampmode', action="store", default="32-true", help="The precision to use. 32-true is default, as per original DiffAE. 32 or 32-true: full precision, 16/16-mixed: half precision (noAMP/AMP), bf16/bf16-mixed: bfloat16 (noAMP/AMP). 64 is also possible, but why to use?")
    ...
    #this model specific
    parser.add_argument("--ema_decay", action="store", default=0.9999, type=float, help="The decay for EMA")
    
    #For post-DiffAE latent classification model
    parser.add_argument('--parent_trainID', action="store", default="", help="Relative path, from the project root, to the configuration yaml file")
    parser.add_argument('--parent_outfolder', action="store", default="Output_fullDSV2", help="Relative path, from the project root, to the configuration yaml file")
    ...
    #Training params
    parser.add_argument('--num_epochs', action="store", default=500, type=int, help="Total number of epochs. If resuming, then it will continue till a total number of epochs set by this.")
    parser.add_argument('--lr', action="store", default=1e-3, type=float)
    parser.add_argument('--lossID', action="store", default=1, type=int, help=f"Loss IDs: {str(LOSSID_CLS)}")
    ...
    #Model Params
    parser.add_argument('--modelID', action="store", default=0, type=int, help="-1: Identity (only for debugging), 0: Single Linear layer")
    ...
    parser.add_argument("-wnbp", "--wnbproject", default='UKBBCardiacImLatent', help="WandB: Name of the project")
    parser.add_argument("-wnbe", "--wnbentity", default='ukbb_img_latent', help="WandB: Name of the entity")
    parser.add_argument("-wnbg", "--wnbgroup", default='latentClsDiffAE', help="WandB: Name of the group")
