...
def getARGSParser():
    ...
    #Basic params
    parser.add_argument('--taskID', action="store", type=int, default=0, help="0: Reconstruction-based latent extraction using AEs and VAEs, 1: Latent extraction using diffusion autoencoder, 2: Latent classification models (utilised for latent manipulation for DiffAE)")
    parser.add_argument('--trainID', action="store", default="prova1", help="The name of the training session. run_prefix (from datainfo json) and foldID will be added as prefix and suffix")
    ...
    parser.add_argument('--batch_size', action="store", default=32, type=int, help="Actual batch size for the model. This can be modified as required, just control the effective_batch_size for comparibility")
    parser.add_argument('--effective_batch_size', action="store", default=96, type=int, help="To make runs with different BS comparible, keep this constant. accumulate_gradbatch would be this divided by BS")
    parser.add_argument('--auto_optim', action=argparse.BooleanOptionalAction, default=False, help="Whether to use Lightning's auto-optimisation or not")
    ...
    parser.add_argument('--n_val_batches', action="store", default=0.25, type=float, help="How much of validation dataset to use, can be float between 0.0 and 1.0, or an integer (float = fraction, int = num_batches)")
    parser.add_argument('--nbatch_prefetch', action="store", default=2, type=int, help=" The number of batches to prefetch for the dataloaders by each worker.")
    ...
    parser.add_argument('--ampmode', action="store", default="32", help="The precision to use. 32 or 32-true: full precision, 16/16-mixed: half precision (noAMP/AMP), bf16/bf16-mixed: bfloat16 (noAMP/AMP). 64 is also possible, but why to use?")
    ...
    parser.add_argument('--norm_type', action="store", default="divbymaxvol", help='Currently "zscore" normalisation and 2 (min-)max modes and their volumetric versions are supported. minmax, divbymax. Volumetric versions: minmaxvol, divbymaxvol')
    parser.add_argument('--zscore_mean', action="store", default="", help="Coma-seperated channel-wise mean values (to be used for zscore normalisation)")
    parser.add_argument('--zscore_std', action="store", default="", help="Coma-seperated channel-wise std values (to be used for zscore normalisation)")
    parser.add_argument('--zscore_prenorm', action="store", default="", help="Integer or float value, to be used as pre-normalisation for zscore normalisation (typically, 255 for 8-bit images). Can also be a string, containing one of the possible values of norm_type (except zscore)")
    ...
    parser.add_argument('--input_shape', action="store", default="56,128,128", help="length, width, depth (to be used if patch_size is not given)")
