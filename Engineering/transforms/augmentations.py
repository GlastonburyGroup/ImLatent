...
class TVTransformWrapper(SuperTransformer):
    def __init__(self, torchV_trans_obj, **kwargs):
        super().__init__(**kwargs)
        self.transformer = torchV_trans_obj

    def apply(self, inp):
        if isinstance(inp, np.ndarray):
            return self.transformer(torch.from_numpy(inp)).numpy()
