...
class ResBlock(nn.Module):
    def __init__(self, in_channels, out_channels, conv_layer=nn.Conv2d):
...
class Encoder_ResNet_AE_MED(BaseEncoder):
    """
    ...
    .. code-block::
        ...
        >>> from pythae.models.nn.benchmarks.celeba import Encoder_ResNet_AE_MED
        >>> from pythae.models import AEConfig
        >>> model_config = AEConfig(input_dim=(3, 64, 64), latent_dim=16)
        >>> encoder = Encoder_ResNet_AE_MED(model_config)
        >>> encoder
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        if self.dim == 2:
            self.embedding = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16), args.latent_dim)
        else:
            self.embedding = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), args.latent_dim)
...
class Encoder_ResNet_VAE_MED(BaseEncoder):
    """
    ...
    .. code-block::
        ...
        >>> from pythae.models.nn.benchmarks.celeba import Encoder_ResNet_VAE_MED
        >>> from pythae.models import VAEConfig
        >>> model_config = VAEConfig(input_dim=(3, 64, 64), latent_dim=16)
        >>> encoder = Encoder_ResNet_VAE_MED(model_config)
        >>> encoder
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        if self.dim == 2:
            self.embedding = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16), args.latent_dim)
            self.log_var = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16), args.latent_dim)
        else:
            self.embedding = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), args.latent_dim)
            self.log_var = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), args.latent_dim)
...
class Encoder_ResNet_SVAE_MED(BaseEncoder):
    """
    ...
    .. code-block::
        ...
        >>> from pythae.models.nn.benchmarks.celeba import Encoder_ResNet_SVAE_MED
        >>> from pythae.models import SVAEConfig
        >>> model_config = SVAEConfig(input_dim=(3, 64, 64), latent_dim=16)
        >>> encoder = Encoder_ResNet_SVAE_MED(model_config)
        >>> encoder
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        if self.dim == 2:
            self.embedding = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16), args.latent_dim)
            self.log_concentration = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16), 1)
        else:
            self.embedding = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), args.latent_dim)
            self.log_concentration = nn.Linear((self.n_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), 1)
...
class Encoder_ResNet_VQVAE_MED(BaseEncoder):
    """
    ...
    .. code-block::
        ...
        >>> from pythae.models.nn.benchmarks.celeba import Encoder_ResNet_VQVAE_MED
        >>> from pythae.models import VQVAEConfig
        >>> model_config = VQVAEConfig(input_dim=(3, 64, 64), latent_dim=16)
        >>> encoder = Encoder_ResNet_VQVAE_MED(model_config)
        >>> encoder
...
class Decoder_ResNet_AE_MED(BaseDecoder):
    """
    ...
    .. code-block::
        ...
        >>> from pythae.models.nn.benchmarks.celeba import Decoder_ResNet_AE_MED
        >>> from pythae.models import VAEConfig
        >>> model_config = VAEConfig(input_dim=(3, 64, 64), latent_dim=16)
        >>> decoder = Decoder_ResNet_AE_MED(model_config)
        >>> decoder
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        if self.dim == 2:
            layers.append(nn.Linear(args.latent_dim, (self.n_encoder_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16)))
        else:
            layers.append(nn.Linear(args.latent_dim, (self.n_encoder_features*2) * (self.input_dim[0]//16) * (self.input_dim[1]//16)* (self.input_dim[2]//16)))
        ...
        layers.append(
            nn.Sequential(convtrans_layer(self.n_encoder_features*2, self.n_encoder_features*2, 5, 2, padding=1), nn.Sigmoid())
        )
        ...
        layers.append(
            nn.Sequential(
                convtrans_layer(self.n_encoder_features, self.n_channels, 4, 2, padding=1), nn.Sigmoid()
            )
        )
...
class Decoder_ResNet_VQVAE_MED(BaseDecoder):
    """
    ...
    .. code-block::
        ...
        >>> from pythae.models.nn.benchmarks.celeba import Decoder_ResNet_VQVAE_MED
        >>> from pythae.models import VQVAEConfig
        >>> model_config = VQVAEConfig(input_dim=(3, 64, 64), latent_dim=16)
        >>> decoder = Decoder_ResNet_VQVAE_MED(model_config)
        >>> decoder
