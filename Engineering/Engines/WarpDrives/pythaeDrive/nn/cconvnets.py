...
class Encoder_CVConv_AE_MED(BaseEncoder):
    """
    ...
    .. code-block::
            ...
            >>> from pythae.models.nn.benchmarks.celeba import Encoder_CVConv_AE_MED
            >>> from pythae.models import AEConfig
            >>> model_config = AEConfig(input_dim=(3, 64, 64), latent_dim=64)
            >>> encoder = Encoder_CVConv_AE_MED(model_config)
            >>> encoder
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        conv_layer = ccnn.Conv2d if self.dim == 2 else ccnn.Conv3d
        bn_layer = ccnn.BatchNorm2d if self.dim == 2 else ccnn.BatchNorm3d
        ...
        layers.append(
            nn.Sequential(
                conv_layer(self.n_features, self.n_features*2, 4, 2, padding=1), bn_layer(self.n_features*2), cnn.modReLU()
            )
        )

        layers.append(
            nn.Sequential(
                conv_layer(self.n_features*2, self.n_features*4, 4, 2, padding=1), bn_layer(self.n_features*4), cnn.modReLU()
            )
        )

        layers.append(
            nn.Sequential(
                conv_layer(self.n_features*4, self.n_features*8, 4, 2, padding=1), bn_layer(self.n_features*8), cnn.modReLU()
            )
        )
        ...
        if self.dim == 2:
            self.embedding = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16), args.latent_dim)
        else:
            self.embedding = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), args.latent_dim)
...
class Encoder_CVConv_VAE_MED(BaseEncoder):
    """
    ...
    .. code-block::
            ...
            >>> from pythae.models.nn.benchmarks.celeba import Encoder_CVConv_VAE_MED
            >>> from pythae.models import VAEConfig
            >>> model_config = VAEConfig(input_dim=(3, 64, 64), latent_dim=64)
            >>> encoder = Encoder_CVConv_VAE_MED(model_config)
            >>> encoder
            ...
            ...   (log_pvar): Linear(in_features=16384, out_features=64, bias=True)
            ... )
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        bn_layer = cnn.BatchNorm2d if self.dim == 2 else cnn.BatchNorm3d
        ...
        if self.dim == 2:
            self.embedding = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16), args.latent_dim)
            self.log_var = nn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16) *2, args.latent_dim) #log_covariance should be real-valued. *2 is because the input is originally complex-valued
            self.log_pvar = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16), args.latent_dim)
        else:
            self.embedding = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), args.latent_dim)
            self.log_var = nn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16) *2, args.latent_dim) #log_covariance should be real-valued. *2 is because the input is originally complex-valued
            self.log_pvar = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), args.latent_dim)
    ...
    def forward(self, x: torch.Tensor, output_layer_levels: List[int] = None):
        ...
        for i in range(max_depth):
            ...
            if i + 1 == self.depth:
                output["embedding"] = self.embedding(out.reshape(x.shape[0], -1))
                output["log_covariance"] = self.log_var(torch.view_as_real(out).reshape(x.shape[0], -1))
                output["log_psuedocovariance"] = self.log_pvar(out.reshape(x.shape[0], -1))

        return output


class Encoder_CVConv_SVAE_MED(BaseEncoder):
    """
    ...
    .. code-block::
            ...
            >>> from pythae.models.nn.benchmarks.celeba import Encoder_CVConv_SVAE_MED
            >>> from pythae.models import SVAEConfig
            >>> model_config = SVAEConfig(input_dim=(3, 64, 64), latent_dim=64)
            >>> encoder = Encoder_CVConv_SVAE_MED(model_config)
            >>> encoder
    ...
    def __init__(self, args: BaseAEConfig):
        ...
        sys.exit("inside Encoder_CVConv_SVAE_MED: SVAE is not yet ready for CV-CNN. Need to find how to implement the log_pvar")
        if self.dim == 2:
            self.embedding = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16), args.latent_dim)
            self.log_concentration = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16), 1)
        else:
            self.embedding = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), args.latent_dim)
            self.log_concentration = cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), 1)
...
class Decoder_CVConv_AE_MED(BaseDecoder):
    """
    ...
    .. code-block::
            ...
            >>> from pythae.models.nn.benchmarks.celeba import Decoder_CVConv_AE_MED
            >>> from pythae.models import VAEConfig
            >>> model_config = VAEConfig(input_dim=(3, 64, 64), latent_dim=64)
            >>> decoder = Decoder_CVConv_AE_MED(model_config)
            >>> decoder
            ...
            ...       (1): cnn.Hirose() [Instead of the usual Sigmoid activation function, as modSigmoid will be real-valued and our decoded output should be complex-valued]
            ...     )
            ...   )
            ... )
    ...
    def __init__(self, args: dict):
        ...
        convtrans_layer = cnn.ConvTranspose2d if self.dim == 2 else cnn.ConvTranspose3d
        bn_layer = cnn.BatchNorm2d if self.dim == 2 else cnn.BatchNorm3d

        layers = nn.ModuleList()

        if self.dim == 2:
            layers.append(nn.Sequential(cnn.Linear(args.latent_dim, (self.n_encoder_features*8) * (self.input_dim[0]//8) * (self.input_dim[1]//8))))
        else:
            layers.append(nn.Sequential(cnn.Linear(args.latent_dim, (self.n_encoder_features*8) * (self.input_dim[0]//8) * (self.input_dim[1]//8) * (self.input_dim[2]//8))))
        ...
        layers.append(
            nn.Sequential(
                convtrans_layer(self.n_encoder_features, self.n_channels, 5, 1, padding=1), cnn.Hirose()
            )
        )
...
class Discriminator_CVConv_MED(BaseDiscriminator):
    """
    ...
    .. code-block::
            ...
            >>> from pythae.models.nn.benchmarks.celeba import Discriminator_CVConv_MED
            >>> from pythae.models import VAEGANConfig
            >>> model_config = VAEGANConfig(input_dim=(3, 64, 64), latent_dim=64)
            >>> discriminator = Discriminator_CVConv_MED(model_config)
            >>> discriminator
    ...
    def __init__(self, args: dict):
        ...
        layers.append(
            nn.Sequential(
                conv_layer(self.n_features, self.n_features*2, 4, 2, padding=1), bn_layer(self.n_features*2), cnn.Hirose()
            )
        )
        ...
        if self.dim == 2:
            layers.append(nn.Sequential(cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16), 1), cnn.modSigmoid()))
        else:    
            layers.append(nn.Sequential(cnn.Linear((self.n_features*8) * (self.input_dim[0]//16) * (self.input_dim[1]//16) * (self.input_dim[2]//16), 1), cnn.modSigmoid()))
