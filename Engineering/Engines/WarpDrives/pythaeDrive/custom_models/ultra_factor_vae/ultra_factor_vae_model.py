...
from .ultra_factor_vae_utils import UltraFactorVAEDiscriminator, UltraPredictor, anti_confounder_loss

from ......utilities import PlasmaConduit

class UltraFactorVAE(VAE):
    """
    FactorVAE model.

    Args:
        model_config (UltraFactorVAEConfig): The Variational Autoencoder configuration setting the main
    ...
    def __init__(
        self,
        model_config: UltraFactorVAEConfig,
        ...
        self.discriminator = UltraFactorVAEDiscriminator(latent_dim=model_config.latent_dim)

        self.model_name = "UltraFactorVAE"
        ...
        if self.anti_confounders_strategy == 1:
            self.model_optims.append("model.anti_counfounder_learner")
            self.loss_optims.append("conf_val_loss")
        if self.learn_phenotypes:
    ...
    def forward(self, inputs: BaseDataset, **kwargs) -> ModelOutput:
        ...
        if self.learn_phenotypes:
            ...
            if "phenotypes" in inputs.keys():
                x_pheno = inputs["phenotypes"][idx_1]
        ...
        if self.anti_confounders_strategy and "confounders" in inputs.keys():
            x_conf = inputs["confounders"][idx_1]   
            if self.anti_confounders_strategy == 1:                
                conf_loss = F.mse_loss(conf_pred, x_conf) #maybe try L1?
            elif self.anti_confounders_strategy == 2:
                conf_corr_loss = anti_confounder_loss(z, x_conf)
            else:
